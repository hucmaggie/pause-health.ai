...
const weeks: ChangelogWeek[] = [
  {
    ...
    headline: "MuleSoft Phase 3 opens; Headless 360 audit reaches all-prototype (gaps #2 + #4 closed)",
    intro:
      "Three threads landed in this week. (1) MuleSoft Phase 3 — the /proposal/mulesoft page's Phase 3 (multi-customer fabric) was pilled `future` since launch; 2026-06-26 shipped the first artifact (pause-omh-to-fhir-library v1.0.0 + CloudHub worker 1.0.5 consuming it); 2026-06-27 added the second (pause-jhe-system-api-spec v1.0.0). (2) Headless 360 gap #2 — env-gated `mcp_api` bearer validator on /api/mcp (introspect-first + userinfo fallback; loopback-bearer propagation in the Care Router's MCP host with a structural same-origin guarantee). (3) Headless 360 gap #4 — the third Headless 360 surface (CLI), shipped as a new in-repo `@pause-health/cli` Node package with four commands (`pause health`, `pause providers`, `pause timeline`, `pause intake`) wrapping the same /api/mulesoft/* surface the MCP server exposes. With #4 closed, ALL FOUR audit gaps on /proposal/headless-360 read `prototype` — three are real Salesforce trust-model conformance closures (PKCE, mcp_api scope, Platform Event egress); #4 is the operator surface that completes the REST + MCP + CLI triad.",
    entries: [
      {
        ...
        commits: [
          { sha: "PENDING", label: "cli: ship @pause-health/cli (gap #4 closed; REST + MCP + CLI triad complete)" }
        ],
        status: "shipped"
      },
      {
