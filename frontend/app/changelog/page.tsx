...
const weeks: ChangelogWeek[] = [
  {
    ...
    entries: [
      {
        title: "Provider graph: coverage-aware selection nearly doubles the directory's ZIP-3 reach (532 → 1,055)",
        summary:
          "The national run held 2,000 real menopause-relevant providers, but they were the global top-N by graphScore — which piles into a handful of dense metros, so the committed directory covered only 532 of ~900 ZIP-3 prefixes. Most ZIPs outside those metros dead-ended on general browsing and the relevant-local fallback even though the data to answer them was sitting right there. New opt-in --coverage mode in build.py (`_round_robin_by_zip3`) spends the same non-certified --limit budget for breadth instead of depth: bucket the non-certified survivors by ZIP-3 prefix, order buckets by their strongest provider, then round-robin one provider per bucket per round so every prefix that has anyone contributes its best candidate before any prefix gets a second. Same 2,000-row budget, distinct-prefix coverage nearly doubled — 532 → 1,055 prefixes (and 55 → 117 state/territory codes). Certified providers are always kept (--keep-all-certified is orthogonal), so the agent's menopause=true coverage is identical and the demo personas still resolve to their curated local certified providers; the mode is OFF by default so the old top-N callers and tests are untouched. refresh_national.sh passes --coverage by default (COVERAGE=0 opts out) and records `coverage: true` in the sidecar metadata; regenerated the committed national directory in place (2,015 providers, 15 certified, 1,055 ZIP-3 prefixes, sanctions unchanged at CA 588 / NY 849 / TX 283). 80 provider_ingest tests green (4 new: round-robin maximizes coverage, then deepens by score, no-ops within budget, and the build wiring never narrows coverage vs top-N) + ruff clean; 270 frontend tests + tsc green behind the unchanged provider contract.",
        commits: [
          { sha: "0d7191b", label: "provider-graph: coverage-aware non-certified selection (532 → 1,055 ZIP-3 prefixes)" }
        ],
        status: "shipped"
      },
      {
