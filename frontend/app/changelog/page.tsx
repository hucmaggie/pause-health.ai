...
const weeks: ChangelogWeek[] = [
  {
    ...
    headline: "MuleSoft Phase 3 = 3 Exchange assets; Headless 360 audit reaches all-prototype",
    intro:
      "Big consolidation week. (1) MuleSoft Phase 3 — the /proposal/mulesoft page's Phase 3 (multi-customer fabric) was pilled `future` since launch; this week shipped three Exchange assets in three days. 2026-06-26: pause-omh-to-fhir-library v1.0.0 + CloudHub worker 1.0.5 consuming it (end-to-end dependency story proven). 2026-06-27: pause-jhe-system-api-spec v1.0.0 + pause-dbdp-system-api-spec v1.0.0, the two System-API contracts ready for the Phase 1c Mule implementations. (2) Headless 360 audit gap #2 — env-gated `mcp_api` bearer validator on /api/mcp (introspect-first + userinfo fallback; loopback-bearer propagation in the Care Router's MCP host with a structural same-origin guarantee). (3) Headless 360 audit gap #4 — the third Headless 360 surface (CLI), shipped as a new in-repo `@pause-health/cli` Node package wrapping /api/mulesoft/*. With #4 closed, ALL FOUR audit gaps on /proposal/headless-360 read `prototype`. Audit is structurally complete; the only path to `shipped` per gap is operator-side env-var procurement.",
    entries: [
      {
        ...
        commits: [
          { sha: "PENDING", label: "mulesoft: ship pause-dbdp-system-api-spec v1.0.0 (Phase 3 third asset)" }
        ],
        status: "shipped"
      },
      {
