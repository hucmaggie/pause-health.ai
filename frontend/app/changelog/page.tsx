...
const weeks: ChangelogWeek[] = [
  {
    ...
    headline: "MuleSoft Phase 3 = full API-led tier coverage on Exchange (5 assets); Headless 360 audit reaches all-prototype",
    intro:
      "Big consolidation week. (1) MuleSoft Phase 3 — the /proposal/mulesoft page's Phase 3 (multi-customer fabric) was pilled `future` since launch; this week shipped five Exchange assets. 2026-06-26: pause-omh-to-fhir-library v1.0.0 + CloudHub worker 1.0.5 consuming it (end-to-end dependency story proven). 2026-06-27: four spec-tier assets — pause-jhe-system-api-spec, pause-dbdp-system-api-spec, pause-oura-system-api-spec (the per-wearable template), and pause-ingest-process-api-spec (the orchestration tier that ties them together). With the Process-tier spec published, the full MuleSoft API-led three-tier story is on Exchange — System (JHE + DBDP + Oura) + Process (this ship) + Experience (pause-provider-experience-api-spec, on Exchange since the Phase 1 worker rollout). (2) Headless 360 audit gap #2 — env-gated `mcp_api` bearer validator on /api/mcp (introspect-first + userinfo fallback; loopback-bearer propagation in the Care Router's MCP host with a structural same-origin guarantee). (3) Headless 360 audit gap #4 — the third Headless 360 surface (CLI), shipped as a new in-repo `@pause-health/cli` Node package wrapping /api/mulesoft/*. With #4 closed, ALL FOUR audit gaps on /proposal/headless-360 read `prototype`. Audit is structurally complete; the only path to `shipped` per gap is operator-side env-var procurement.",
    entries: [
      {
        ...
        commits: [
          { sha: "PENDING", label: "mulesoft: ship pause-ingest-process-api-spec v1.0.0 (Phase 3 fifth asset; Process tier)" }
        ],
        status: "shipped"
      },
      {
