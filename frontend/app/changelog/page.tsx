...
const weeks: ChangelogWeek[] = [
  {
    ...
    entries: [
      {
        title: "Agentforce provider-lookup action staged (External Service + Named Credential + runbook)",
        summary:
          "The live Pause_Health_Intake_Agent deflects on \"find a provider that specializes in menopause\" because it's a generic intake-only Service Agent with no action wired to the Pause provider graph — provider lookup was always a \"Designed\" roadmap item, so nothing regressed. Staged the repo-side artifacts that turn the org-side fix into a fill-in-the-blanks job, no Apex required because /api/mulesoft/providers is a public no-auth GET: a lean External-Services-compatible OpenAPI 3.0 spec (operation findMenopauseProviders, stripped of $ref/oneOf/nullable/auth so the parser accepts it), a no-auth Named Credential pointing at https://pause-health.ai, and a runbook that walks registering the External Service, adding a \"Find a Provider\" topic + action to the agent (classification description, instructions, input mapping menopause=true / limit=3 / zip, example utterances), testing, and the deactivate→activate cycle. The runbook also calls out the known prechat no-op: the ZIP captured at intake isn't handed to the agent in-band, so the topic instructs the agent to ask the patient for it (the action also works ZIP-less, returning top national matches). Action goes live once the External Service + topic are registered and the agent is re-activated on the org.",
        commits: [
