...
const weeks: ChangelogWeek[] = [
  {
    ...
    headline: "MuleSoft Phase 3 opens; Headless 360 audit gap #2 closed (`mcp_api` bearer gate on /api/mcp)",
    intro:
      "Two threads landed. (1) MuleSoft Phase 3 — the /proposal/mulesoft page's Phase 3 (multi-customer fabric) was pilled `future` since launch; that assumption was wrong, shared System-API artifacts can land on Exchange the moment they exist. 2026-06-26 shipped the first artifact (pause-omh-to-fhir-library v1.0.0 + CloudHub worker 1.0.5 consuming it); 2026-06-27 added the second (pause-jhe-system-api-spec v1.0.0). (2) Headless 360 audit gap #2 — the `mcp_api` scope on /api/mcp. The audit page at /proposal/headless-360 had this pilled `designed` since the PKCE seam (gap #1) shipped. 2026-06-27 closed it with an env-gated bearer validator (introspect-first, userinfo fallback for orgs that disable introspect; loopback bearer propagation in the Care Router's MCP host; same-origin guard so Salesforce-issued bearers can't leak to external MCP remotes). Two of the four audit gaps now read `prototype`; only the `mcp_api` scope availability and the optional Salesforce CLI parity remain.",
    entries: [
      {
        ...
        commits: [
          { sha: "PENDING", label: "headless-360: ship mcp_api bearer gate on /api/mcp (gap #2 closed)" }
        ],
        status: "shipped"
      },
      {
