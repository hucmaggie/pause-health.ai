import { describe, expect, it, vi } from "vitest";

import type { A2ATask } from "../lib/a2a";
import {
  KPI_PRESETS,
  buildKpiRequestBody,
  kpiViewFromTask,
  runKpiTask
} from "./kpi-trend-panel";

describe("KPI_PRESETS", () => {
  it("has the three fit presets and the three governance-block presets", () => {
    const ids = KPI_PRESETS.map((p) => p.id);
    expect(ids).toContain("rising");
    expect(ids).toContain("flat");
    expect(ids).toContain("declining");
    expect(ids).toContain("phantom-point-block");
    expect(ids).toContain("mis-fit-block");
    expect(ids).toContain("auto-committed-block");
  });

  it("attaches an asserted determination only to the block presets", () => {
    for (const p of KPI_PRESETS) {
      if (p.id.endsWith("-block")) expect(p.determination).toBeDefined();
      else expect(p.determination).toBeUndefined();
    }
  });
});

describe("buildKpiRequestBody", () => {
  it("builds a tasks/send envelope with a request", () => {
    const body = buildKpiRequestBody({
      taskId: "t-1",
      personaId: "demo",
      request: { seriesRef: "s", horizon: 6, flatTolerance: 0.5, observations: [] }
    });
    expect(body.jsonrpc).toBe("2.0");
    expect(body.method).toBe("tasks/send");
    expect(body.params.id).toBe("t-1");
    const data = body.params.message.parts[0].data as Record<string, unknown>;
    expect((data.request as { seriesRef: string }).seriesRef).toBe("s");
    expect(data.determination).toBeUndefined();
  });

  it("includes an asserted determination when supplied", () => {
    const body = buildKpiRequestBody({ taskId: "t-2", determination: { trend: "rising" } });
    const data = body.params.message.parts[0].data as Record<string, unknown>;
    expect(data.determination).toEqual({ trend: "rising" });
  });
});

describe("runKpiTask", () => {
  it("POSTs and returns the A2A task result", async () => {
    const task: A2ATask = {
      id: "t-9",
      status: { state: "completed", timestamp: "now", message: { role: "agent", parts: [] } }
    };
    const fetchImpl = vi.fn(async () =>
      new Response(JSON.stringify({ jsonrpc: "2.0", id: "t-9", result: task }), {
        status: 200,
        headers: { "Content-Type": "application/json" }
      })
    ) as unknown as typeof fetch;
    const result = await runKpiTask({ taskId: "t-9" }, fetchImpl);
    expect(result.id).toBe("t-9");
  });

  it("throws on an RPC error", async () => {
    const fetchImpl = vi.fn(async () =>
      new Response(JSON.stringify({ jsonrpc: "2.0", id: "t", error: { code: -32600, message: "bad" } }), {
        status: 200,
        headers: { "Content-Type": "application/json" }
      })
    ) as unknown as typeof fetch;
    await expect(runKpiTask({ taskId: "t" }, fetchImpl)).rejects.toThrow("bad");
  });
});

describe("kpiViewFromTask", () => {
  it("lifts a resolved view from a completed task", () => {
    const task: A2ATask = {
      id: "t-1",
      status: { state: "completed", timestamp: "now", message: { role: "agent", parts: [] } },
      artifacts: [
        {
          name: "KpiTrendDetermination",
          index: 0,
          parts: [
            {
              type: "data",
              data: {
                result: {
                  seriesRef: "kpi-provider-org-adoption",
                  determination: {
                    seriesRef: "kpi-provider-org-adoption",
                    trend: "rising",
                    slope: 11,
                    intercept: 20,
                    rSquared: 1,
                    projection: 86,
                    horizon: 6,
                    points: [{ index: 0, value: 20, fitted: 20, residual: 0 }],
                    reason: "r",
                    note: "n"
                  }
                }
              }
            }
          ]
        }
      ],
      metadata: {
        agentFabric: {
          decision: "allow",
          traceTaskId: "t-1",
          kpiSeriesSourced: true,
          kpiFitConsistent: true,
          kpiNoAutonomousCommit: true
        }
      }
    };
    const view = kpiViewFromTask(task);
    expect(view.kind).toBe("resolved");
    if (view.kind === "resolved") {
      expect(view.trend).toBe("rising");
      expect(view.slope).toBe(11);
      expect(view.projection).toBe(86);
      expect(view.points).toHaveLength(1);
      expect(view.kpiFitConsistent).toBe(true);
    }
  });

  it("lifts a blocked view from a governance block", () => {
    const task: A2ATask = {
      id: "t-2",
      status: {
        state: "failed",
        timestamp: "now",
        message: { role: "agent", parts: [{ type: "text", text: "blocked" }] }
      },
      metadata: {
        agentFabric: {
          decision: "block",
          policiesEvaluated: ["policy.kpi.fit-consistent"],
          violations: [{ policyId: "policy.kpi.fit-consistent", reason: "mis-fit" }],
          traceTaskId: "t-2"
        }
      }
    };
    const view = kpiViewFromTask(task);
    expect(view.kind).toBe("blocked");
    if (view.kind === "blocked") {
      expect(view.violations[0].policyId).toBe("policy.kpi.fit-consistent");
    }
  });

  it("lifts an invalid view from a non-block failure", () => {
    const task: A2ATask = {
      id: "t-3",
      status: {
        state: "failed",
        timestamp: "now",
        message: { role: "agent", parts: [{ type: "text", text: "nope" }] }
      },
      metadata: { agentFabric: { decision: "invalid", traceTaskId: "t-3" } }
    };
    const view = kpiViewFromTask(task);
    expect(view.kind).toBe("invalid");
  });
});
