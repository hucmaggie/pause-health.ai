import { describe, expect, it, vi } from "vitest";

import type { A2ATask } from "../lib/a2a";
import {
  HOUSEHOLD_COMPOSITION_PRESETS,
  buildHouseholdCompositionRequestBody,
  householdCompositionViewFromTask,
  runHouseholdCompositionTask
} from "./household-composition-panel";

describe("HOUSEHOLD_COMPOSITION_PRESETS", () => {
  it("has the three finding presets and the three governance-block presets", () => {
    const ids = HOUSEHOLD_COMPOSITION_PRESETS.map((p) => p.id);
    expect(ids).toContain("households-formed");
    expect(ids).toContain("transitive-chain");
    expect(ids).toContain("all-singletons");
    expect(ids).toContain("phantom-link-block");
    expect(ids).toContain("mis-grouped-block");
    expect(ids).toContain("auto-merged-block");
  });

  it("attaches an asserted determination only to the block presets", () => {
    for (const p of HOUSEHOLD_COMPOSITION_PRESETS) {
      if (p.id.endsWith("-block")) expect(p.determination).toBeDefined();
      else expect(p.determination).toBeUndefined();
    }
  });
});

describe("buildHouseholdCompositionRequestBody", () => {
  it("builds a tasks/send envelope with a request", () => {
    const body = buildHouseholdCompositionRequestBody({
      taskId: "t-1",
      personaId: "demo",
      request: {
        batchRef: "b",
        members: ["m1", "m2"],
        links: [{ a: "m1", b: "m2", basis: "shared-subscriber" }]
      }
    });
    expect(body.jsonrpc).toBe("2.0");
    expect(body.method).toBe("tasks/send");
    expect(body.params.id).toBe("t-1");
    const data = body.params.message.parts[0].data as Record<string, unknown>;
    expect((data.request as { members: string[] }).members).toEqual(["m1", "m2"]);
    expect(data.determination).toBeUndefined();
  });

  it("includes an asserted determination when supplied", () => {
    const body = buildHouseholdCompositionRequestBody({
      taskId: "t-2",
      determination: { disposition: "households-formed" }
    });
    const data = body.params.message.parts[0].data as Record<string, unknown>;
    expect(data.determination).toEqual({ disposition: "households-formed" });
  });
});

describe("runHouseholdCompositionTask", () => {
  it("POSTs and returns the A2A task result", async () => {
    const task: A2ATask = {
      id: "t-9",
      status: { state: "completed", timestamp: "now", message: { role: "agent", parts: [] } }
    };
    const fetchImpl = vi.fn(async () =>
      new Response(JSON.stringify({ jsonrpc: "2.0", id: "t-9", result: task }), {
        status: 200,
        headers: { "Content-Type": "application/json" }
      })
    ) as unknown as typeof fetch;
    const result = await runHouseholdCompositionTask({ taskId: "t-9" }, fetchImpl);
    expect(result.id).toBe("t-9");
  });

  it("throws on an RPC error", async () => {
    const fetchImpl = vi.fn(async () =>
      new Response(JSON.stringify({ jsonrpc: "2.0", id: "t", error: { code: -32600, message: "bad" } }), {
        status: 200,
        headers: { "Content-Type": "application/json" }
      })
    ) as unknown as typeof fetch;
    await expect(runHouseholdCompositionTask({ taskId: "t" }, fetchImpl)).rejects.toThrow("bad");
  });
});

describe("householdCompositionViewFromTask", () => {
  it("lifts a resolved view from a completed task", () => {
    const task: A2ATask = {
      id: "t-1",
      status: { state: "completed", timestamp: "now", message: { role: "agent", parts: [] } },
      artifacts: [
        {
          name: "HouseholdCompositionDetermination",
          index: 0,
          parts: [
            {
              type: "data",
              data: {
                result: {
                  batchRef: "hh-batch-001",
                  determination: {
                    batchRef: "hh-batch-001",
                    members: ["m1", "m2", "m3", "m4", "m5", "m6"],
                    links: [
                      { a: "m1", b: "m2", basis: "shared-subscriber" },
                      { a: "m2", b: "m3", basis: "shared-address" },
                      { a: "m4", b: "m5", basis: "shared-subscriber" }
                    ],
                    households: [
                      { householdId: "hh-1", members: ["m1", "m2", "m3"], size: 3 },
                      { householdId: "hh-2", members: ["m4", "m5"], size: 2 },
                      { householdId: "hh-3", members: ["m6"], size: 1 }
                    ],
                    householdCount: 3,
                    largestHouseholdSize: 3,
                    memberCount: 6,
                    disposition: "households-formed",
                    reason: "r",
                    note: "n"
                  }
                }
              }
            }
          ]
        }
      ],
      metadata: {
        agentFabric: {
          decision: "allow",
          traceTaskId: "t-1",
          householdLinksSourced: true,
          householdPartitionConsistent: true,
          householdNoAutonomousMerge: true
        }
      }
    };
    const view = householdCompositionViewFromTask(task);
    expect(view.kind).toBe("resolved");
    if (view.kind === "resolved") {
      expect(view.disposition).toBe("households-formed");
      expect(view.householdCount).toBe(3);
      expect(view.households).toHaveLength(3);
      expect(view.linkCount).toBe(3);
      expect(view.householdLinksSourced).toBe(true);
    }
  });

  it("lifts a blocked view from a governance block", () => {
    const task: A2ATask = {
      id: "t-2",
      status: {
        state: "failed",
        timestamp: "now",
        message: { role: "agent", parts: [{ type: "text", text: "blocked" }] }
      },
      metadata: {
        agentFabric: {
          decision: "block",
          policiesEvaluated: ["policy.household.partition-consistent"],
          violations: [{ policyId: "policy.household.partition-consistent", reason: "bad grouping" }],
          traceTaskId: "t-2"
        }
      }
    };
    const view = householdCompositionViewFromTask(task);
    expect(view.kind).toBe("blocked");
    if (view.kind === "blocked") {
      expect(view.violations[0].policyId).toBe("policy.household.partition-consistent");
    }
  });

  it("lifts an invalid view from a non-block failure", () => {
    const task: A2ATask = {
      id: "t-3",
      status: {
        state: "failed",
        timestamp: "now",
        message: { role: "agent", parts: [{ type: "text", text: "nope" }] }
      },
      metadata: { agentFabric: { decision: "invalid", traceTaskId: "t-3" } }
    };
    const view = householdCompositionViewFromTask(task);
    expect(view.kind).toBe("invalid");
  });
});
