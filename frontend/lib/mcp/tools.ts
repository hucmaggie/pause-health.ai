 ...
 *
 * NOTE: This file is duplicated from `mcp/src/tools.ts`. The two
 * copies MUST stay byte-identical — that's enforced by the parity test
 * at `frontend/lib/mcp/tools.parity.test.ts`, which fails CI if they
 * drift. Edit `mcp/src/tools.ts` as the canonical copy, then `cp` it
 * here. The duplication exists because frontend/ and mcp/ are separate
 * npm packages (no workspace), and a relative TS import would break
 * the standalone `npm publish` path.
 *
