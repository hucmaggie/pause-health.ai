# Pause Care Plan · Claude Sonnet 4.5

Anypoint Exchange **Agents** asset (A2A v0.3 card, classifier `a2a-card`).

- **Fabric agent id:** `care-plan-agent`
- **Artifact:** `pause-agent-care-plan-agent` `1.0.0` (groupId `56707cc3-a0e3-4318-b110-78126aace370`)
...
- **Policies enforced (6):** policy.audit.hipaa-log-every-turn, policy.careplan.template-sourced, policy.model.anthropic-claude-sonnet-allowlisted, policy.clinical.no-prescribing, policy.clinical.rationale-required, policy.data360.consent-required-before-grounding
