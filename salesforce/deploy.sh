...
#
# NOTE: the permission set grants FLS on the full ~21-field dossier, but only
# Pause_Patient_Zip__c is tracked in this repo today. That deploys cleanly to an
# org where the other dossier fields already exist (e.g. trailsignup). For a
# BRAND-NEW org, run ./retrieve.sh first to pull the remaining fields + the
# Agent (Bot/GenAiPlannerBundle), or the permission set deploy will fail on the
# undefined field references. See README.md.
set -euo pipefail
