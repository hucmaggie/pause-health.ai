...
> agent + topic + action in the UI, then `./retrieve.sh` and commit — the repo
> becomes source-of-truth AFTER the UI authors it.

### 2a. Verified action-wiring reference (decoded from the live intake agent)

The intake agent's `Find_a_Provider` topic wires the `PauseProviderDirectory`
...
In the UI you don't type this — you add the `verifyBenefits` action to the topic
and map `payer`/`zip` to the bot context variables; the UI emits the above. The
